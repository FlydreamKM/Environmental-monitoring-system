...
static void ST7789_WriteCommandNoCS(uint8_t cmd)
{
	ST7789_DC_Clr();
	ST7789_SPI_WRITE(&cmd, sizeof(cmd));
}
...
static void ST7789_WriteDataNoCS(uint8_t *buff, size_t buff_size)
{
	ST7789_DC_Set();
	while (buff_size > 0) {
		uint16_t chunk_size = buff_size > 65534 ? 65534 : buff_size;
...
static void ST7789_WriteSmallDataNoCS(uint8_t data)
{
	ST7789_DC_Set();
	ST7789_SPI_WRITE(&data, sizeof(data));
}
...
void ST7789_SetRotation(uint8_t m)
{
	...
	switch (m) {
	case 0:
		ST7789_WriteSmallData(ST7789_MADCTL_MX | ST7789_MADCTL_MY | ST7789_MADCTL_BGR);
		break;
	case 1:
		ST7789_WriteSmallData(ST7789_MADCTL_MY | ST7789_MADCTL_MV | ST7789_MADCTL_BGR);
		break;
	case 2:
		ST7789_WriteSmallData(ST7789_MADCTL_BGR);
		break;
	case 3:
		ST7789_WriteSmallData(ST7789_MADCTL_MX | ST7789_MADCTL_MV | ST7789_MADCTL_BGR);
		break;
	default:
		break;
	}
...
static void ST7789_SetAddressWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
	...
	/* Column Address set */
	ST7789_WriteCommandNoCS(ST7789_CASET); 
	{
		uint8_t data[] = {x_start >> 8, x_start & 0xFF, x_end >> 8, x_end & 0xFF};
		ST7789_WriteDataNoCS(data, sizeof(data));
	}

	/* Row Address set */
	ST7789_WriteCommandNoCS(ST7789_RASET);
	{
		uint8_t data[] = {y_start >> 8, y_start & 0xFF, y_end >> 8, y_end & 0xFF};
		ST7789_WriteDataNoCS(data, sizeof(data));
	}
	/* Write to RAM */
	ST7789_WriteCommandNoCS(ST7789_RAMWR);
	ST7789_UnSelect();
}
...
void ST7789_Init(void)
{
    ...
    ST7789_WriteSmallData(0x28);			//	0.85v for 240x320
...
void ST7789_Test(void)
{
	...
	//	Image test removed to save Flash space (saber was 32KB)
	//	If you need image display, load from external storage or use smaller arrays.
}
