...
void ST7789_DrawQuarterCircle(uint16_t x0, uint16_t y0, uint8_t r, uint16_t color, uint8_t quadrant) {
    ...
    if (quadrant == 1) { // 第一象限
        y0 -= r;
        x0 += r;
    } else if (quadrant == 2) { // 第二象限
        x0 -= r;
        y0 -= r;
    } else if (quadrant == 3) { // 第三象限
        y0 += r;
        x0 -= r;
    } else if (quadrant == 4) { // 第四象限
        y0 += r;
        x0 += r;
    }

    ST7789_Select();
    ST7789_DrawPixel(x0, y0 + y, color); // 起始点
...
void ST7789_DrawRoundRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t r, uint16_t color) {
    // 确保宽度和高度足够容纳圆角
    if (w < r * 2 || h < r * 2) {
        return; // 如果宽度或高度小于圆角直径，则无法绘制圆角
    }

    // 绘制四条边
    ST7789_DrawLine(x + r, y, x + w - r, y, color); // 上边
    ST7789_DrawLine(x + r, y + h, x + w - r, y + h, color); // 下边
    ST7789_DrawLine(x, y + r, x, y + h - r, color); // 左边
    ST7789_DrawLine(x + w, y + r, x + w, y + h - r, color); // 右边

    // 绘制四个圆角
    ST7789_DrawQuarterCircle(x + r, y + r, r, color, 1); // 左上角
    ST7789_DrawQuarterCircle(x + w - r - 1, y + r, r, color, 2); // 右上角
    ST7789_DrawQuarterCircle(x + w - r - 1, y + h - r - 1, r, color, 3); // 右下角
    ST7789_DrawQuarterCircle(x + r, y + h - r - 1, r, color, 4); // 左下角
}
...
void ST7789_FillRoundRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t r, uint16_t color) {
    ...
    // 绘制填充的矩形主体部分
    ST7789_DrawFilledRectangle(x + r, y, w - r * 2, h, color);

    // 绘制四个圆角
    ST7789_FillQuarterCircle(x + r, y + r, r, color, 1); // 左上角
    ST7789_FillQuarterCircle(x + w - r - 1, y + r, r, color, 2); // 右上角
    ST7789_FillQuarterCircle(x + w - r - 1, y + h - r - 1, r, color, 3); // 右下角
    ST7789_FillQuarterCircle(x + r, y + h - r - 1, r, color, 4); // 左下角
}
...
void ST7789_FillQuarterCircle(uint16_t x0, uint16_t y0, uint8_t r, uint16_t color, uint8_t quadrant) {
    ...
    while (x < y) {
        ...
        // 根据象限绘制填充
        if (quadrant == 1) { // 第一象限
            ST7789_DrawFilledRectangle(x0 - x, y0 - y, 2 * x + 1, y + 1, color);
            ST7789_DrawFilledRectangle(x0 - y, y0 - x, 2 * y + 1, x + 1, color);
        } else if (quadrant == 2) { // 第二象限
            ST7789_DrawFilledRectangle(x0 - x, y0 + y - 1, 2 * x + 1, y + 1, color);
            ST7789_DrawFilledRectangle(x0 - y, y0 + x - 1, 2 * y + 1, x + 1, color);
        } else if (quadrant == 3) { // 第三象限
            ST7789_DrawFilledRectangle(x0 + y - 1, y0 + x - 1, 2 * y + 1, x + 1, color);
            ST7789_DrawFilledRectangle(x0 + x - 1, y0 + y - 1, 2 * x + 1, y + 1, color);
        } else if (quadrant == 4) { // 第四象限
            ST7789_DrawFilledRectangle(x0 + y - 1, y0 - x, 2 * y + 1, x + 1, color);
            ST7789_DrawFilledRectangle(x0 + x - 1, y0 - y, 2 * x + 1, y + 1, color);
        }
    }

    if (r > 0) { // 最后填充半径为1的圆角
        if (quadrant == 1 || quadrant == 4) {
            ST7789_DrawFilledRectangle(x0 - r, y0 - r, r + r + 1, r + r + 1, color);
