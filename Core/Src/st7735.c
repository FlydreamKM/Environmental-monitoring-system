...
static struct {
    ...
    uint16_t rows_done;
    uint8_t active_buf;
    const uint16_t *image_src;
    uint16_t image_stride;
    ST7735_DMA_Mode mode;
} st7735_dma_state;
...
void HAL_SPI_TxCpltCallback(SPI_HandleTypeDef *hspi) {
    ...
    if(st7735_dma_state.rows_done < st7735_dma_state.rows_total) {
        ...
        if(st7735_dma_state.mode == ST7735_DMA_IMAGE) {
            ...
            const uint8_t *src = (const uint8_t*)(st7735_dma_state.image_src + next_row * st7735_dma_state.image_stride);
            memcpy(st7735_buf[next_buf], src, st7735_dma_state.w * 2);
        }
        // FILL mode: buffers already contain the same color, no need to refill
